...
export default function StoreEditDialog({ open, onClose, store, onUpdated }) {
  const [error, setError] = useState('');

  const handleSubmit = async (values, { setSubmitting }) => {
    setError('');
    try {
      await storesApi.updateStore(store.id, values);
      onUpdated();
    } catch (err) {
      setError(err.response?.data?.message || err.response?.data || 'Failed to update store');
    } finally {
      setSubmitting(false);
    }
  };
            ...
            <DialogContent>
              {error && (
                <Alert severity="error" sx={{ mb: 2 }}>
                  {typeof error === 'string' ? error : 'Failed to update store'}
                </Alert>
              )}
